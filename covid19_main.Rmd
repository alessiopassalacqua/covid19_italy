...
coronavirus %>%
  ...
  ggplot(aes(x=date,y=cases_tot,color=type)) + geom_line()
```
...
```{r}
totali_china <- xts( coronavirus_china$cases_tot ,(coronavirus_china$date + 35)  )
totali_italy <- totali
```


```{r}
chinaVsitaly  <- merge.xts(totali_italy, totali_china)
```

```{r}
dygraph(chinaVsitaly) %>%
  dyRangeSelector() 
```
